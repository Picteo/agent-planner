"""
Clan War (CW) service for AliceIsBored Discord bot.

Handles fetching CW (non-CWL Clan War) data from Supercell API
...
from database import CwEvents, CwParticipations, Clan, DatabaseManager
...
class CwService:
    """Fetches CW (Clan War) data from Supercell API and persists it to the database."""

    def __init__(self, api_client: "SupercellAPIClient", db_manager: DatabaseManager):
        self._api_client = api_client
        self._db = db_manager

    async def sync_cw(self, clan_tag: str) -> dict:
        """Fetch CW data for a clan and persist to database.

        Args:
            clan_tag: Clan tag to fetch CW data for (with or without '#').
        ...
        try:
            ...
            cw_data = await self._api_client.get_clan_wars(clan.clan_tag)
            if cw_data is None:
                return {"error": f"No CW data available for clan #{clean_tag}"}

            wars = cw_data.get("war", [])
            past_wars = cw_data.get("pastWars", [])
            all_wars = wars + past_wars
            total_events = 0
            total_participations = 0

            for war in all_wars:
                result = await self._process_war(session, clan, war)
                total_events += 1
        ...
        logger.info(
            "CW sync complete for clan #%s: %d events, %d participations",
    ...
    async def _process_war(self, session, clan, war: dict) -> dict:
        """Process a single CW war and persist participation records."""
        war_state = war.get("state", "")
        start_raw = war.get("start", "")
        end_raw = war.get("end", "")
        ...
        event = CwEvents(
            ...
            end_time=end_time,
            attack_days=war.get("attackDays", 1),
        )
        session.add(event)
        session.flush()

        day_groups = war.get("dayGroups", [])
        ...
        for day_group in day_groups:
            ...
            for player_group in player_groups:
                ...
                participation = CwParticipations(
                    event_id=event.id,
                    player_tag=player_tag,
                    day_number=day_number,
                    attacks_used=player_group.get("attacksUsed", 0),
                    attack_targets=player_group.get("attackTargets", 0),
