"""
Raid (Challenge) service for AliceIsBored Discord bot.

Handles fetching Raid data from Supercell API
...
from database import Clan, DatabaseManager, RaidEvents, RaidParticipations
...
class RaidService:
    """Fetches Raid data from Supercell API and persists it to the database."""

    def __init__(self, api_client: "SupercellAPIClient", db_manager: DatabaseManager):
        self._api_client = api_client
        self._db = db_manager

    async def sync_raids(self, clan_tag: str) -> dict:
        """Fetch Raid data for a clan and persist to database.

        Args:
            clan_tag: Clan tag to fetch Raid data for (with or without '#').
        ...
        try:
            ...
            challenges = raid_data.get("challenges", [])
            total_events = 0
        ...
        logger.info(
            "Raid sync complete for clan #%s: %d events, %d participations",
    ...
    async def _process_challenge(self, session, challenge: dict) -> dict:
        """Process a single Raid challenge and persist participation records."""
        start_raw = challenge.get("startTime", "")
        end_raw = challenge.get("endTime", "")
        ...
        for player_group in player_groups:
            ...
            participation = RaidParticipations(
                event_id=event.id,
                player_tag=player_tag,
                attacks_used=player_group.get("attacksUsed", 0),
                points_reached=player_group.get("pointsReached", 0),
            )
